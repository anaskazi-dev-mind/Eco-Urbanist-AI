...
const ImageComparisonSlider = ({ beforeImage, afterImage, beforeLabel = "Original", afterLabel = "AI Enhanced" }) => {
  ...
  return (
    <div className="w-full">
      ...
      >
        ...
        >
          <div className={`w-16 h-16 bg-white rounded-full shadow-2xl flex items-center justify-center cursor-ew-resize border-4 transition-all duration-300 ${
            isDragging 
              ? 'border-green-500 scale-125 shadow-green-500/50' 
              : 'border-gray-300 hover:border-green-400 hover:scale-110'
          }`}>
            <MoveHorizontal className={`w-8 h-8 transition-colors ${isDragging ? 'text-green-600' : 'text-gray-600'}`} />
          </div>
        </div>
