...
const GalleryCard = ({ item, onDelete }) => {
  ...
  const handleDownload = async () => {
    try {
      ...
      alert('Failed to download image. The file may no longer exist on the server.');
    }
  };
          ...
          <div className="absolute bottom-3 left-3 right-3">
            <div className="flex items-center justify-between text-white text-sm">
              <div className="flex items-center gap-1">
                ...
                <span className="font-bold">{inputScore.toFixed(1)}% → {outputScore.toFixed(1)}%</span>
              </div>
              <div className="flex items-center gap-1">
      ...
      <div className="p-5">
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-gray-600 font-semibold">Improvement</span>
            <span className={`text-2xl font-black ${improvement > 20 ? 'text-green-600' : improvement > 10 ? 'text-emerald-600' : 'text-gray-700'}`}>
        ...
        <div className="bg-green-50 rounded-xl p-3 mb-4 border border-green-200">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-bold text-gray-700">Trees Placed</span>
            </div>
            <span className="text-xl font-black text-green-600">{treesPlaced}</span>
          </div>
        </div>
