...
import { Image as ImageIcon, Search, Filter, SortDesc, Trash2, TrendingUp, TreePine, Calendar, AlertCircle, Upload as UploadIcon } from 'lucide-react';
...
const Gallery = () => {
  ...
  const filterAndSortGallery = () => {
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'improvement':
          return (b.greenScores?.improvement || 0) - (a.greenScores?.improvement || 0);
        case 'trees':
          return (b.visualization?.trees_placed || 0) - (a.visualization?.trees_placed || 0);
        default:
          return 0;
      }
    });
  ...
  const handleClearAll = () => {
    if (window.confirm('⚠️ Are you sure you want to delete ALL results? This cannot be undone!')) {
      ...
      <div className="container-custom max-w-7xl">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center bg-white shadow-md border border-green-200 px-4 py-2 rounded-full mb-6">
            ...
            <span className="text-sm font-bold text-green-700">Your Transformations</span>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-10">
          <div className="bg-white rounded-2xl shadow-xl p-6 border border-gray-100 hover:shadow-2xl transition-shadow">
            ...
            <p className="text-4xl font-black text-gray-900 mb-1">{stats.totalImages}</p>
            <p className="text-sm text-gray-600 font-semibold">Images Processed</p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6 border border-gray-100 hover:shadow-2xl transition-shadow">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs text-gray-500 font-bold uppercase">Average</span>
            </div>
            <p className="text-4xl font-black text-green-600 mb-1">
              +{stats.averageImprovement.toFixed(1)}%
            </p>
            <p className="text-sm text-gray-600 font-semibold">Green Improvement</p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6 border border-gray-100 hover:shadow-2xl transition-shadow">
            ...
            <p className="text-4xl font-black text-emerald-600 mb-1">{stats.totalTreesPlanted}</p>
            ...
            {gallery.length === 0 ? (
              <>
                ...
                <p className="text-gray-600 mb-8 max-w-md mx-auto">
                  Your gallery is empty. Upload your first satellite image to start creating green transformations!
                </p>
                <Link
                  to="/upload"
